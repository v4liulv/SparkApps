...
class PairDStreamFunctions[K, V](self: DStream[(K, V)])
   ...
   * org.org.apache.spark.Partitioner is used to control the partitioning of each RDD.
   */
  ...
  /**
   * Return a new DStream by applying `reduceByKey` to each RDD. The values for each key are
   * merged using the supplied reduce function. org.org.apache.spark.Partitioner is used to control
   ...
   * org.org.apache.spark.rdd.PairRDDFunctions in the Spark core documentation for more information.
   */
  ...
  /**
   * Return a new DStream by applying 'cogroup' between RDDs of `this` DStream and `other` DStream.
   * The supplied org.org.apache.spark.Partitioner is used to partition the generated RDDs.
   */
  def cogroup[W: ClassTag](
      other: DStream[(K, W)],
      partitioner: Partitioner
  ...
  /**
   * Return a new DStream by applying 'join' between RDDs of `this` DStream and `other` DStream.
   * The supplied org.org.apache.spark.Partitioner is used to control the partitioning of each RDD.
   */
  def join[W: ClassTag](
      other: DStream[(K, W)],
      partitioner: Partitioner
  ...
  /**
   * Return a new DStream by applying 'left outer join' between RDDs of `this` DStream and
   * `other` DStream. The supplied org.org.apache.spark.Partitioner is used to control
   * the partitioning of each RDD.
   */
  def leftOuterJoin[W: ClassTag](
      other: DStream[(K, W)],
  ...
  def saveAsHadoopFile[F <: OutputFormat[K, V]](
    ...
    saveAsHadoopFile(path, keyClass, valueClass,
  ...
  def saveAsHadoopFile(
    ...
    val saveFunc = (rdd: RDD[(K, V)], time: Time) => {
      rdd.saveAsHadoopFile(outputPath, keyClass, valueClass, outputFormatClass,
  ...
  def saveAsNewAPIHadoopFile(
    ...
    val saveFunc = (rdd: RDD[(K, V)], time: Time) => {
      rdd.saveAsNewAPIHadoopFile(
        path, keyClass, valueClass, outputFormatClass, serializableConf.value)
    }
    self.foreachRDD(saveFunc)
  }
