...
object WordCount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
      ...
      * */
    //本地模式
    // val lines = sc.textFile("G://文档//Spark//源码//spark1.6.0//spark-1.6.0-bin-hadoop2.6//README.md", 1) //读取本地文件，并设置为Partition
    /** 集群 通过参数进行传递HDFS路径，如果没有此路径就默认路径hdfs://hadoop04.com.cn:8020/tmp/CHANGES.txt **/
    val lines =   if (args != null && args.length == 1 ) sc.textFile(args(0))  else  sc.textFile("hdfs://hadoop04.com.cn:8020/tmp/CHANGES.txt")
    ...
    /**
      ...
      */
    val wordCounts = pairs.reduceByKey(_ + _).map(pairs => (pairs._2, pairs._1)).sortByKey(ascending = false).map(pairs => (pairs._2, pairs._1))
